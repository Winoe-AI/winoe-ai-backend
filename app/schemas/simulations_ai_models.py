...
from app.schemas.simulations_ai_values import normalize_eval_enabled_by_day
...
class SimulationCompanyContext(BaseModel):
    ...
    domain: str | None = Field(default=None, min_length=1, max_length=MAX_COMPANY_CONTEXT_VALUE_CHARS)
...
class SimulationAIConfig(BaseModel):
    ...
    notice_version: str | None = Field(default=None, alias="noticeVersion", min_length=1, max_length=MAX_AI_NOTICE_VERSION_CHARS)
    notice_text: str | None = Field(default=None, alias="noticeText", min_length=1, max_length=MAX_AI_NOTICE_TEXT_CHARS)
    eval_enabled_by_day: dict[str, StrictBool] | None = Field(default=None, alias="evalEnabledByDay")
