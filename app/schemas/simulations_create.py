...
class SimulationCreate(BaseModel):
    ...
    seniority: str = Field(..., min_length=1, max_length=100, validation_alias=AliasChoices("seniority", "roleLevel", "role_level"))
    focus: str = Field(..., min_length=1, max_length=MAX_FOCUS_NOTES_CHARS, validation_alias=AliasChoices("focus", "focusNotes", "focus_notes"))
    company_context: SimulationCompanyContext | None = Field(default=None, alias="companyContext")
    ai: SimulationAIConfig | None = None
    templateKey: str = Field(DEFAULT_TEMPLATE_KEY, min_length=1, max_length=255, description="Template key")
    ...
    def _validate_day_window_bounds(self):
        ...
        if self.dayWindowOverrides and not self.dayWindowOverridesEnabled:
            raise ValueError("dayWindowOverridesEnabled must be true when dayWindowOverrides is set")
        return self
