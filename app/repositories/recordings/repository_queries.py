...
async def get_by_id_for_update(db: AsyncSession, recording_id: int) -> RecordingAsset | None:
    stmt = select(RecordingAsset).where(RecordingAsset.id == recording_id).with_for_update()
    return (await db.execute(stmt)).scalar_one_or_none()
