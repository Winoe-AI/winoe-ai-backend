...
def normalize_day_score_payload(
    ...
    for idx, entry in enumerate(day_scores):
        ...
        day_index = coerce_day_index(entry.get("day_index"), field_path=f"{field_path}.day_index")
        if day_index in seen_day_indexes:
            raise ValueError(f"day_scores contains duplicate day_index values: {day_index}")
        seen_day_indexes.add(day_index)
        normalized.append(
            {
                "day_index": day_index,
                "score": coerce_score(entry.get("score"), field_path=f"{field_path}.score"),
                "rubric_results_json": coerce_rubric_results_json(
                    entry.get("rubric_results_json"),
                    field_path=f"{field_path}.rubric_results_json",
                ),
                "evidence_pointers_json": validate_evidence_pointers(entry.get("evidence_pointers_json")),
            }
        )
    return normalized
...
async def add_day_scores(
    ...
    *,
    ...
    normalized_entries = normalize_day_score_payload(day_scores, allow_empty=allow_empty)
    if not normalized_entries:
        await (db.commit() if commit else db.flush())
        return []
    existing_day_indexes = set((await db.execute(select(EvaluationDayScore.day_index).where(EvaluationDayScore.run_id == run.id))).scalars())
    duplicates = sorted({int(entry["day_index"]) for entry in normalized_entries if int(entry["day_index"]) in existing_day_indexes})
    if duplicates:
