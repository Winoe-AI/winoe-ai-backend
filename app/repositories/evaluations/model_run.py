...
from sqlalchemy import CheckConstraint, DateTime, Float, ForeignKey, Index, Integer, JSON, String, func
...
class EvaluationRun(Base):
    ...
    __table_args__ = (
        CheckConstraint(status_check_expr(), name=EVALUATION_RUN_STATUS_CHECK_CONSTRAINT_NAME),
        CheckConstraint("completed_at IS NULL OR completed_at >= started_at", name=EVALUATION_RUN_COMPLETED_AT_CHECK_CONSTRAINT_NAME),
        CheckConstraint(recommendation_check_expr(), name=EVALUATION_RUN_RECOMMENDATION_CHECK_CONSTRAINT_NAME),
        Index("ix_evaluation_runs_candidate_session_scenario_version", "candidate_session_id", "scenario_version_id"),
        Index("ix_evaluation_runs_candidate_session_started_at", "candidate_session_id", "started_at"),
        Index("ix_evaluation_runs_candidate_session_status_started_at", "candidate_session_id", "status", "started_at"),
    ...
    scenario_version_id: Mapped[int] = mapped_column(ForeignKey("scenario_versions.id", ondelete="CASCADE"), nullable=False)
    status: Mapped[str] = mapped_column(String(50), nullable=False, default=EVALUATION_RUN_STATUS_PENDING, server_default=EVALUATION_RUN_STATUS_PENDING)
    ...
    candidate_session = relationship("CandidateSession", back_populates="evaluation_runs")
    scenario_version = relationship("ScenarioVersion")
    day_scores = relationship("EvaluationDayScore", back_populates="run", cascade="all, delete-orphan", order_by="EvaluationDayScore.day_index")
