...
from app.repositories.evaluations.repository_queries import get_run_by_id
...
async def create_run_with_day_scores(
    ...
    *,
    ...
    await add_day_scores(db, run=run, day_scores=day_scores, allow_empty=False, commit=False)
    if commit:
        await db.commit()
