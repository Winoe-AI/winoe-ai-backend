    ...
    try:
        with urlopen_fn(request, timeout=5) as response:
            ...
            if content_length_raw is None:
                raise storage_error_cls("Storage object metadata missing Content-Length header")
            try:
                size_bytes = int(content_length_raw)
            except ValueError as exc:
                raise storage_error_cls("Storage object metadata returned invalid Content-Length") from exc
            if size_bytes < 0:
                raise storage_error_cls("Storage object metadata returned negative Content-Length")
            content_type = response.headers.get("Content-Type", "").split(";", 1)[0].strip().lower()
            if not content_type:
                raise storage_error_cls("Storage object metadata missing Content-Type header")
            return metadata_cls(content_type=content_type, size_bytes=size_bytes)
    except HTTPError as exc:
        if exc.code == 404:
            return None
        raise storage_error_cls(f"Failed to inspect storage object metadata (status={exc.code})") from exc
...
def delete_object(
    ...
    *,
    ...
    try:
        ...
        if exc.code == 404:
            return
        raise storage_error_cls(f"Failed to delete storage object (status={exc.code})") from exc
    except OSError as exc:
