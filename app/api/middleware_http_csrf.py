...
logger = logging.getLogger("app.api.middleware_http")


class CsrfOriginEnforcementMiddleware:
    def __init__(self, app, *, allowed_origins: list[str] | None = None, protected_path_prefixes: list[str] | None = None) -> None:
        self.app = app
        self.allowed_origins = {
            normalized for origin in (allowed_origins or []) if (normalized := _normalize_origin(origin))
        }
        self.protected_path_prefixes = [
            prefix for prefix in (_normalize_path_prefix(p) for p in (protected_path_prefixes or [])) if prefix
        ]
    ...
    async def __call__(self, scope, receive, send):
        ...
        logger.warning(
            ...
            extra={"method": method, "path": path, "origin": raw_origin or None, "referer_origin": referer_origin},
        )
        response = JSONResponse(status_code=status.HTTP_403_FORBIDDEN, content=dict(_CSRF_ORIGIN_MISMATCH_RESPONSE))
        await response(scope, receive, send)


from .middleware_http_config import _normalize_path_prefix
