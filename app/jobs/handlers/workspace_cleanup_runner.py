...
from app.jobs.handlers.workspace_cleanup_processing import _process_cleanup_target
...
async def handle_workspace_cleanup_impl(payload_json: dict[str, Any], **deps) -> dict[str, Any]:
    ...
    async with deps["async_session_maker"]() as db:
        ...
        deps["logger"].info("workspace_cleanup_started", extra={"jobId": job_id, "companyId": company_id, "countCandidates": summary["candidateCount"]})
        candidate_session_ids = sorted({target.candidate_session.id for target in targets})
        ...
        for target in targets:
            await _process_cleanup_target(
                ...
                job_id=job_id,
                logger=deps["logger"],
                enforce_collaborator_revocation=deps["_enforce_collaborator_revocation"],
