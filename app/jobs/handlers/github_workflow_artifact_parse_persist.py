...
from .github_workflow_artifact_parse_payload import ArtifactParsePayload
...
async def persist_artifact_parse_result(
    *,
    ...
    async with async_session_maker() as db:
        submission = (await db.execute(select(Submission).where(Submission.id == payload.submission_id).with_for_update())).scalar_one_or_none()
        ...
        if payload.payload_task_id is not None and payload.payload_task_id != submission.task_id:
            return {
        ...
        if stored_workflow_run_id and stored_workflow_run_id != str(payload.workflow_run_id):
            return {
        ...
        workspace = (await db.execute(select(Workspace).where(Workspace.candidate_session_id == submission.candidate_session_id, Workspace.task_id == submission.task_id).with_for_update())).scalar_one_or_none()

        runner, github_client = build_actions_runner()
        try:
            result = await runner.fetch_run_result(repo_full_name=payload.repo_full_name, run_id=payload.workflow_run_id)
        finally:
        ...
        normalized_conclusion = normalized_conclusion.lower() if normalized_conclusion else None
