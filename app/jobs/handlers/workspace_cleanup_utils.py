...
def _cleanup_is_complete(record: WorkspaceCleanupRecord) -> bool:
    return record.cleanup_status in WORKSPACE_CLEANUP_TERMINAL_STATUSES and record.cleaned_at is not None
...
def _cleanup_target_repo_key(*, candidate_session_id: int, repo_full_name: str | None, fallback_id: str) -> tuple[int, str]:
