...
from app.jobs.handlers.workspace_cleanup_processing_status import (
...
async def _process_cleanup_target(
    *,
    ...
    record.retention_expires_at = _retention_expires_at(_retention_anchor(record, candidate_session), retention_days=config.retention_days)
    try:
        ...
        if revoke_status in {"collaborator_removed", "collaborator_already_removed"}:
            ...
            await _mark_revocation_blocked(db=db, summary=summary, record=record, job_id=job_id, logger=logger)
            return

        if not _retention_cleanup_eligible(candidate_session=candidate_session, simulation=simulation):
        ...
        await db.commit()
        logger.warning("workspace_cleanup_failed", extra={"jobId": job_id, "repoFullName": exc.repo_full_name, "workspaceId": exc.workspace_id, "errorCode": exc.error_code})
