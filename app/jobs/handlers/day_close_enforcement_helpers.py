...
async def _resolve_default_branch(github_client, *, repo_full_name: str, workspace_default_branch: str | None) -> str:
...
async def _revoke_repo_write_access(github_client, *, repo_full_name: str, github_username: str | None, candidate_session_id: int, day_index: int, logger: logging.Logger) -> str:
    username = (github_username or "").strip()
    if not username:
        logger.warning("day_close_enforcement_missing_github_username", extra={"candidateSessionId": candidate_session_id, "repoFullName": repo_full_name, "dayIndex": day_index})
