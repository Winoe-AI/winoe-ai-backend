"""Application module for evaluations services evaluations winoe report pipeline queries service workflows."""
...
async def _tasks_by_day(db: AsyncSession, *, trial_id: int) -> dict[int, Task]:
    tasks = (
        (
            await db.execute(
                select(Task)
