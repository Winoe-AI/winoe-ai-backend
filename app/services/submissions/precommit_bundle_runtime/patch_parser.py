...
from app.services.submissions.precommit_bundle_runtime.models import BundleFileChange


def parse_patch_entries(*, patch_text: str | None, storage_ref: str | None) -> list[BundleFileChange]:
    ...
    for idx, raw_entry in enumerate(entries_raw):
        ...
        changes.append(BundleFileChange(path=path, content=content, delete=delete, executable=executable))
    return changes
...
def _parse_content(raw_entry: dict[str, object], idx: int, path: str, delete: bool) -> str | None:
    if delete:
        return None
...
def ensure_safe_repo_path(path: str) -> None:
    ...
    if path.startswith("/") or path.startswith("./") or path.startswith("../"):
        _raise_unsafe_path(path, "Precommit bundle path cannot be absolute or traversing.")
    if "\\" in path:
