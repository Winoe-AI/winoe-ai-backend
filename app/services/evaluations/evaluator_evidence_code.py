...
from app.services.evaluations.evaluator_helpers import _safe_repo_full_name
...
def _build_code_day_evidence(day: DayEvaluationInput) -> list[dict[str, Any]]:
    ...
    if commit_ref is not None:
        ...
        if repo_full_name:
            commit_item["url"] = f"https://github.com/{repo_full_name}/commit/{commit_ref}"
    ...
    if isinstance(base_ref, str) and isinstance(head_ref, str):
        ...
        if repo_full_name:
            diff_item["url"] = f"https://github.com/{repo_full_name}/compare/{base_ref}...{head_ref}"
    ...
    if isinstance(day.tests_passed, int) or isinstance(day.tests_failed, int):
        ...
        test_item: dict[str, Any] = {
            "kind": "test",
            "ref": str(day.workflow_run_id or day.submission_id or f"day-{day.day_index}"),
            "excerpt": f"Tests summary: passed={passed}, failed={failed}.",
        }
        if repo_full_name and isinstance(day.workflow_run_id, str) and day.workflow_run_id.strip():
            test_item["url"] = f"https://github.com/{repo_full_name}/actions/runs/{day.workflow_run_id.strip()}"
