...
def _compose_day_scores(run: EvaluationRun, persisted_report: Mapping[str, Any]) -> list[dict[str, Any]]:
    scored_by_day: dict[int, dict[str, Any]] = {}
    for row in sorted(run.day_scores, key=lambda r: r.day_index):
        evidence = [
            sanitized
            for sanitized in (_sanitize_evidence(pointer) for pointer in (row.evidence_pointers_json or []))
