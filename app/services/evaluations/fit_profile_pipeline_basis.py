...
from app.domains import CandidateDayAudit, Submission, Transcript
...
def _submission_basis_hash(submission: Submission | None) -> str | None:
    ...
    submission_basis = {
        ...
        "lastRunAt": submission.last_run_at.isoformat() if submission.last_run_at else None,
    }
...
def _build_basis_references(
    *,
    ...
    day_refs = {
        str(day_index): {
            "submissionId": (
                submissions_by_day[day_index].id if day_index in submissions_by_day else None
            ),
            "submissionBasisHash": _submission_basis_hash(submissions_by_day.get(day_index)),
            "cutoffCommitSha": day_audits[day_index].cutoff_commit_sha if day_index in day_audits else None,
            "evalBasisRef": day_audits[day_index].eval_basis_ref if day_index in day_audits else None,
        }
