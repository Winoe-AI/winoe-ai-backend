...
from app.repositories.evaluations.models import EVALUATION_RUN_STATUS_COMPLETED, EvaluationRun
...
async def complete_run(
    ...
    *,
    ...
    ensure_transition(current_status=run.status, target_status=EVALUATION_RUN_STATUS_COMPLETED)
    await evaluation_repo.add_day_scores(db, run=run, day_scores=day_scores, allow_empty=allow_empty_day_scores, commit=False)
    ...
    if resolved_generated_at < started_at:
        raise EvaluationRunStateError("generated_at must be greater than or equal to started_at.")
    run.status = EVALUATION_RUN_STATUS_COMPLETED
    run.completed_at = resolved_completed_at
    run.generated_at = resolved_generated_at
    run.overall_fit_score = coerce_unit_interval_score(overall_fit_score, field_name="overall_fit_score", required=False)
    run.recommendation = coerce_recommendation(recommendation, required=False)
    run.confidence = coerce_unit_interval_score(confidence, field_name="confidence", required=False)
