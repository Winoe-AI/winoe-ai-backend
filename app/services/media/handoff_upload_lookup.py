...
async def resolve_company_id(
    ...
    *,
    ...
    if simulation is not None and isinstance(getattr(simulation, "company_id", None), int):
        return simulation.company_id
    company_id = (
        await db.execute(select(Simulation.company_id).where(Simulation.id == simulation_id))
    ).scalar_one_or_none()
    if not isinstance(company_id, int):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
