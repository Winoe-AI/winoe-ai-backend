...
def _normalize_overrides(overrides: Mapping[Any, Any] | None) -> dict[int, tuple[time, time]]:
    ...
    for raw_day, raw_window in overrides.items():
        ...
        if day_index is None or not isinstance(raw_window, Mapping):
            continue
        raw_start = raw_window.get("startLocal") or raw_window.get("windowStartLocal") or raw_window.get("start")
        raw_end = raw_window.get("endLocal") or raw_window.get("windowEndLocal") or raw_window.get("end")
