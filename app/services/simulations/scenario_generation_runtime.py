...
from app.services.simulations.scenario_generation_payloads import (
    build_rubric_json,
    build_task_prompts_json,
)
from app.services.simulations.scenario_generation_story import (
...
def _task_description_for_day(
    day_index: int, role: str, tech_stack: str, template_key: str, *, pick: PickFn, template_display_name: TemplateNameFn
) -> str:
...
def build_deterministic_template_scenario(
    *, role: str, tech_stack: str, template_key: str, pick: PickFn, template_display_name: TemplateNameFn
) -> GeneratedScenarioPayload:
...
def generate_scenario_payload(
    *,
    ...
    if source == SCENARIO_SOURCE_TEMPLATE_FALLBACK:
        return build_fallback(role=role, tech_stack=tech_stack, template_key=template_key)
    try:
        return generate_with_llm(role=role, tech_stack=tech_stack, template_key=template_key)
