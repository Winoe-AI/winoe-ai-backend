...
from app.services.simulations.scenario_generation_text import normalize_text, seed_from_inputs
...
def build_task_description(
    *,
    ...
    if day_index == 1:
        return "Draft an implementation plan that defines service boundaries, key data flows, API contracts, and risk controls. Include concrete tradeoffs and how you will validate correctness in {stack_label}.".format(stack_label=stack_label)
    if day_index == 2:
        return "Implement the primary backend slice in code with tests. Prioritize {priority} and keep the solution aligned with the {template_name} project structure.".format(priority=priority, template_name=template_name)
    if day_index == 3:
        return "Investigate and fix a failing behavior path. Treat {debug_signal} as the anchor signal, isolate root cause, and add regression coverage.".format(debug_signal=debug_signal)
    if day_index == 4:
