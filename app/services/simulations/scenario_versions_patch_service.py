...
from app.domains import ScenarioEditAudit, ScenarioVersion, Simulation
...
from app.services.simulations.scenario_versions_patch_utils import (
...
async def patch_scenario_version(
    ...
    *,
    ...
    scenario_version = await _require_scenario_for_patch(db, scenario_version_id, simulation)
    ensure_patch_allowed(simulation, scenario_version, actor_user_id)
    before_state = snapshot_editable_state(scenario_version)
    candidate_fields = [field for field in SCENARIO_PATCH_FIELD_ORDER if field in updates]
...
async def _require_scenario_for_patch(
    db: AsyncSession, scenario_version_id: int, simulation: Simulation
) -> ScenarioVersion:
    scenario_version = await scenario_repo.get_by_id(db, scenario_version_id, for_update=True)
