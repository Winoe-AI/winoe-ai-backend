"""Application module for trials routes trials routes trials routes list trials routes workflows."""
...
@router.get("", response_model=list[TrialListItem], status_code=status.HTTP_200_OK)
async def list_trials(
    db: Annotated[AsyncSession, Depends(get_session)],
    user: Annotated[Any, Depends(get_current_user)],
    includeTerminated: bool = False,
):
    """List trials for Talent Partner dashboard (scoped to current user)."""
    ensure_talent_partner_or_none(user)
    rows = await sim_service.list_trials(
    ...
    return [
        TrialListItem(
            ...
            status=sim_service.normalize_trial_status_or_raise(
