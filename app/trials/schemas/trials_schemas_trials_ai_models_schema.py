"""Application module for trials schemas trials ai models schema workflows."""
...
class TrialCompanyContext(BaseModel):
...
class TrialAIAgentRuntimeSummary(BaseModel):
    """TalentPartner-visible summary of one frozen agent policy/runtime."""
...
class TrialAISnapshotSummary(BaseModel):
    ...
    agents: list[TrialAIAgentRuntimeSummary] | None = None
...
class TrialAIConfig(BaseModel):
    """AI notice/toggle controls for a trial."""
    ...
    active_scenario_snapshot: TrialAISnapshotSummary | None = Field(
        default=None,
        alias="activeScenarioSnapshot",
    )
    pending_scenario_snapshot: TrialAISnapshotSummary | None = Field(
        default=None,
...
class TrialDayWindowOverride(BaseModel):
