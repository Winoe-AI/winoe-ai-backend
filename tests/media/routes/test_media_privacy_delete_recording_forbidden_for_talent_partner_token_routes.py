...
@pytest.mark.asyncio
async def test_delete_recording_forbidden_for_talent_partner_token(
    async_client,
    async_session,
    candidate_header_factory,
):
    talent_partner = await create_talent_partner(
        async_session, email="privacy-delete-talent_partner@test.com"
    )
    ...
    owner_session = await create_candidate_session(
        async_session,
        trial=sim,
        invite_email="delete-owner-talent_partner@test.com",
        status="in_progress",
    )
    recording = await _seed_uploaded_recording(
        ...
        task_id=task.id,
        filename="forbidden-talent_partner.mp4",
    )
    ...
    response = await async_client.post(
        ...
        headers=candidate_header_factory(
            owner_session,
            token=f"talent_partner:{talent_partner.email}",
        ),
    )
