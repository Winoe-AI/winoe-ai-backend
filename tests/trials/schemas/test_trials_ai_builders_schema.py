...
from app.trials.schemas import (
    trials_schemas_trials_ai_builders_schema as ai_builders,
)


def test_build_trial_company_context_handles_success_and_invalid_shapes():
    assert ai_builders.build_trial_company_context("not-a-map") is None
    assert ai_builders.build_trial_company_context({"unknown": "value"}) is None

    context = ai_builders.build_trial_company_context(
...
def test_build_trial_ai_config_returns_valid_resolved_config(monkeypatch):
    monkeypatch.setattr(
        ai_builders,
    ...
    config = ai_builders.build_trial_ai_config(
        notice_version=None,
        notice_text=None,
...
def test_build_trial_ai_config_falls_back_when_resolved_values_do_not_validate(
    monkeypatch,
):
    monkeypatch.setattr(
        ai_builders,
