...
def test_template_display_name_handles_blank_catalog_value(monkeypatch):
    monkeypatch.setattr(scenario_generation, "TEMPLATE_CATALOG", {"template-x": {"display_name": "   "}})
...
def test_apply_generated_task_updates_covers_invalid_and_blank_inputs():
    tasks = [
        type("Task", (), {"day_index": 1, "title": "Original 1", "description": "Original 1", "max_score": 5})(),
        type("Task", (), {"day_index": 2, "title": "Original 2", "description": "Original 2", "max_score": 7})(),
    ]
...
def test_apply_generated_task_updates_handles_non_dict_day_weights():
    task = type("Task", (), {"day_index": 3, "title": "Title", "description": "Desc", "max_score": 9})()
    scenario_generation.apply_generated_task_updates(
        tasks=[task],
        task_prompts_json=[],
...
def test_normalize_transcript_segments_keeps_text_when_present():
    normalized = fit_profile_pipeline._normalize_transcript_segments([{"startMs": 1, "endMs": 2, "text": "hello"}])
    assert normalized == [{"startMs": 1, "endMs": 2, "text": "hello"}]


def test_normalize_transcript_segments_allows_missing_text():
    normalized = fit_profile_pipeline._normalize_transcript_segments([{"startMs": 1, "endMs": 2, "text": "   "}])
