...
async def test_build_ready_payload_uses_persisted_run_shape(async_session):
    recruiter = await create_recruiter(async_session, email="fit-composer@test.com")
    ...
    run = await evaluation_repo.create_run_with_day_scores(
        ...
        raw_report_json={"overallFitScore": 0.78},
        day_scores=[
            {
                "day_index": 2,
                "score": 0.75,
