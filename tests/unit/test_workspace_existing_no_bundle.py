...
async def test_ensure_existing_workspace_records_no_bundle_details(monkeypatch):
    ...
    async def _apply_bundle(*_args, **_kwargs):
        return SimpleNamespace(
            ...
            precommit_sha=None,
            bundle_id=None,
            details={"reason": "bundle_not_found", "scenarioVersionId": 22, "templateKey": "template-default"},
        )
    ...
    monkeypatch.setattr(ws_existing.workspace_repo, "set_precommit_details", _set_precommit_details)
...
async def test_ensure_existing_workspace_returns_existing_when_no_updates(monkeypatch):
    ...
    async def _apply_bundle(*_args, **_kwargs):
        return SimpleNamespace(state="no_bundle", precommit_sha=None, bundle_id=None, details={"reason": "bundle_not_found"})
