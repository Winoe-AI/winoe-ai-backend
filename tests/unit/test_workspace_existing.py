...
def test_serialize_no_bundle_details_handles_non_matching_inputs():
    assert ws_existing._serialize_no_bundle_details(SimpleNamespace(state="applied", details={"reason": "commit_created"})) is None
    assert ws_existing._serialize_no_bundle_details(SimpleNamespace(state="no_bundle", details=["bad-payload"])) is None


@pytest.mark.asyncio
async def test_ensure_existing_workspace_hydrates_missing_precommit(monkeypatch):
    existing = SimpleNamespace(repo_full_name="org/repo", default_branch="main", base_template_sha="base-sha", precommit_sha=None)
    calls: dict[str, str] = {}
    ...
    async def _apply_bundle(*_args, **_kwargs):
        return SimpleNamespace(state="applied", precommit_sha="precommit-sha", bundle_id=1)
    ...
    monkeypatch.setattr(ws_existing.workspace_repo, "set_precommit_sha", _set_precommit_sha)
