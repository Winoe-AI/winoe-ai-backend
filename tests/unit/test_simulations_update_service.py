...
class _DummyDB:
    ...
    async def commit(self) -> None: self.commit_calls += 1

    async def refresh(self, instance: object) -> None: self.refresh_calls.append(instance)
...
async def test_update_simulation_logs_ai_changes_without_notice_text(
    ...
    monkeypatch.setattr(sim_update_service, "require_owned_simulation_with_tasks", _fake_require_owned_with_tasks)
    ...
    payload = SimpleNamespace(
        ai=SimpleNamespace(
            ...
            eval_enabled_by_day={"1": True, "2": False, "3": True, "4": True, "5": False},
        )
    )
    db = _DummyDB()
    ...
    assert simulation.ai_eval_enabled_by_day == {"1": True, "2": False, "3": True, "4": True, "5": False}
    assert db.commit_calls == 1
    assert len(db.refresh_calls) == 3

    log_text = "\n".join(record.getMessage() for record in caplog.records)
    assert "simulation_ai_notice_version_changed simulationId=42 actorUserId=7" in log_text
    assert "from=mvp1 to=mvp2" in log_text
    assert "simulation_ai_eval_toggles_changed simulationId=42 actorUserId=7" in log_text
