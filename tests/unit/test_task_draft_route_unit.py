...
async def test_get_task_draft_route_success(monkeypatch):
    ...
    response = await draft_route.get_task_draft_route(task_id=33, candidate_session=candidate_session, db=object())
...
async def test_get_task_draft_route_not_found(monkeypatch):
    ...
    with pytest.raises(ApiError) as excinfo:
        await draft_route.get_task_draft_route(task_id=33, candidate_session=candidate_session, db=object())
