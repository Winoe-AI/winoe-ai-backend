...
class StubGithubClient:
    ...
    async def generate_repo_from_template(
        ...
        *,
        ...
        return {"full_name": f"{owner_prefix}/{new_repo_name}", "id": 999, "default_branch": "main"}

    async def add_collaborator(self, repo_full_name: str, username: str, *, permission: str = "push"):
        return {"ok": True}
    ...
    async def get_file_contents(self, repo_full_name: str, file_path: str, *, ref: str | None = None):
    ...
    async def list_commits(self, repo_full_name: str, *, sha: str | None = None, per_page: int = 30):
        return []
    ...
    async def create_blob(self, repo_full_name: str, *, content: str, encoding: str = "utf-8"):
        return {"sha": f"blob-{len(content.encode('utf-8'))}"}

    async def create_tree(self, repo_full_name: str, *, tree: list[dict], base_tree: str | None = None):
        return {"sha": "tree-sha-456", "tree": tree, "base_tree": base_tree}

    async def create_commit(self, repo_full_name: str, *, message: str, tree: str, parents: list[str]):
        return {"sha": "precommit-sha-789", "message": message, "tree": tree}

    async def update_ref(self, repo_full_name: str, *, ref: str, sha: str, force: bool = False):
