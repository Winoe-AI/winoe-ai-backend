...
from tests.recruiters.services.recruiters_admin_ops_utils import *


@pytest.mark.asyncio
async def test_set_candidate_session_day_window_dry_run_is_non_mutating(async_session):
    recruiter = await create_recruiter(async_session, email="day-window-dry@test.com")
...
async def test_set_candidate_session_day_window_requires_claimed_session(async_session):
    recruiter = await create_recruiter(async_session, email="day-window-guard@test.com")
    simulation, _ = await create_simulation(async_session, created_by=recruiter)
    candidate_session = await create_candidate_session(
        async_session,
        simulation=simulation,
