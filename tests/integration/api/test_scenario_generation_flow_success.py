...
@pytest.mark.asyncio
async def test_scenario_generation_job_creates_v1_and_updates_detail_read(async_client, async_session, auth_header_factory, monkeypatch):
    ...
    try:
        ...
        handled = await worker.run_once(session_maker=session_maker(async_session), worker_id="scenario-api-worker", now=datetime.now(UTC))
    finally:
        worker.clear_handlers()
    ...
    async with session_maker(async_session)() as check_session:
        ...
        scenario_v1 = (await check_session.execute(select(ScenarioVersion).where(ScenarioVersion.simulation_id == simulation_id, ScenarioVersion.version_index == 1))).scalar_one()
        task_rows = ((await check_session.execute(select(Task).where(Task.simulation_id == simulation_id).order_by(Task.day_index.asc()))).scalars().all())
    ...
    assert refreshed_job.idempotency_key == f"simulation:{simulation_id}:scenario_generation"
    assert scenario_v1.storyline_md and scenario_v1.task_prompts_json and isinstance(scenario_v1.rubric_json, dict)
    ...
    detail_response = await async_client.get(f"/api/simulations/{simulation_id}", headers=auth_header_factory(recruiter))
    ...
    prompts_by_day = {int(prompt["dayIndex"]): prompt for prompt in detail["scenario"]["taskPromptsJson"]}
    ...
    for day_index in [1, 2, 3, 4, 5]:
        assert tasks_by_day[day_index]["description"] == prompts_by_day[day_index]["description"]
