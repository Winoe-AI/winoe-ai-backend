...
from app.simulations.services import (
    simulations_services_simulations_codespace_specializer_service as specializer_service,
)
...
async def test_ensure_precommit_bundle_ready_for_invites_treats_retrying_bundle_as_not_ready(
    ...
    with pytest.raises(ApiError) as exc_info:
        await specializer_service.ensure_precommit_bundle_ready_for_invites(
            db=_Db(),
            simulation=SimpleNamespace(company_id=7),
...
async def test_run_codespace_specializer_job_degrades_to_context_bundle_after_retryable_provider_failures(
    ...
    simulation = SimpleNamespace(id=31, company_id=7)
    ...
    class _Db:
        ...
        async def execute(self, _stmt):
            ...
            if self.execute_calls == 1:
                return _ScalarResult(simulation)
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: SimpleNamespace(
            patch_payload_json='{"files":[{"path":"TENON_SIMULATION_CONTEXT.md"}]}',
