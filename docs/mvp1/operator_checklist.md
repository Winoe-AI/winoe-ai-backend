...
Use this checklist before invites, during active evaluations, and before sharing fit-profile outputs.

## A. Pre-invite controls

- [ ] Confirm an active scenario version is set for the simulation (`active_scenario_version_id`).
- [ ] Confirm the active scenario version is invite-ready/locked before sending invites.
- [ ] Confirm AI notice/toggle policy is correct on the simulation:
  - `ai_notice_version`
  - `ai_notice_text`
...
- [ ] Confirm submission/recruiter views show cutoff basis fields (`cutoffCommitSha`, `evalBasisRef`) where applicable.
- [ ] Confirm fit-profile run metadata contains basis data (`basisRefs`, `basis_fingerprint`) for traceability.
...
- [ ] Re-run evaluation (`POST /api/candidate_sessions/{candidate_session_id}/fit_profile/generate`) after any scenario-version change that should affect scoring context.
- [ ] Re-run evaluation after any change that affects cutoff basis for day 2/3.
- [ ] Confirm the latest run is the one used for review (`GET /api/candidate_sessions/{candidate_session_id}/fit_profile`).
